...
//for test purposes
let string_test = [1, 2, 3];

//give me only the names of all the ancestors, using ECMAScript6
var names = ancestry.map((x) => x.name); //ECMAScript6


//now filter only the male ancestors of this beautiful farmer family
var filterSex = function(ancestry){
  return ancestry.sex === 'm'
}

//then, composing it into a filter higher-order function that I learned
var filter = ancestry.filter(filterSex);

//now I want to test the super higher-order function called reduce()
//in order to test it, I'm gonna sum up all dying ages, as I currently can't figure out how to calculate each age, and sum 'em up
var getDyingAges = ancestry.reduce(function(result, item){
  //console.loggin all intermediate results to check if it works out correctly
  console.log('your sum is going well, currently its ' + result + ' now adding ' + item["died"]);

  return result + item.died
}, 0)

//now to show the same DyingAges function in ECMAScript6...
var getDyingAgesECMA = ancestry.reduce((result, item) => result + item.died, 0)


//all console.logs to check if they work out well... Obviously
console.log('all dying ages summed up are: ' + getDyingAges);
console.log('the ECMAScript6 dying ages are the same, but faster ;) : ' + getDyingAgesECMA);
console.log('all the males present in the family tree are: ');
console.log(filter);
console.log('these are the names of each family member: ' + names);